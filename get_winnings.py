...
def get_winnings(driver):
    ...
    if no_winnings_text == "You do not have any winning bets!":
        ...
        winnings_odds = 0
        roi = 0
        input_winnings(round_number, winnings_odds, winnings_amount, roi)
    # Save HTML of winnings page to parse and get numbers to assign to variables (to input into db)
        ...
        fetch_bet_tuple = cur.execute("SELECT amount_bet FROM food_club WHERE round = ?", (round_number,))
        ...
        input_winnings(round_number, winnings_odds, total_winnings, roi)
